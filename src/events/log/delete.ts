...
const selfDeleteLog = new DiscordEventBuilder({
  ...
  execute: async (message) => {
    if (!message.inGuild() || message.author.app) return;
...
const deleteLog = new DiscordEventBuilder({
  ...
  execute: async (auditLog, guild) => {
    ...
    if (!auditLog.target.inGuild() || auditLog.target.author.app) return;
...
async function sendDeleteLog(message: Message<true>, channel?: GuildBasedChannel | null, executor?: User | null) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription([
      ...
      `${formatEmoji(Emojis.Gray.member)} **Supprimé par :** ${executor ? `${executor} [\`${executor.tag}\`]` : 'Expéditeur lui-même'}`,
